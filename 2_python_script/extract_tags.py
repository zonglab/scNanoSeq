...
***HpyCH4V/AluI library
  ...
  Possible sequences NNNTCAR(145), NNNTCTR(145), NNNXTCAR(144), NNNXTCTR(144)
...
class ExtractTags:
  ...
  def extract_tags(self):
    ...
    for header1 in self.in1:
        ### create 2 read objects
        r1 = Read(self.readlength)
        r2 = Read(self.readlength)
        # header
        r1.add_header(header1)
        r2.add_header(self.in2.next())
        if r1.header != r2.header:
            raise ValueError('File is not in sync')
        # sequence
        r1.add_sequence(self.in1.next())
        r2.add_sequence(self.in2.next())
        # spacer
        self.in1.next()
        self.in2.next()
        # quality
        r1.add_quality(self.in1.next())
        r2.add_quality(self.in2.next())
        ...
        if r1.tag in self.tags and r2.tag in self.tags:
          r1.trim_sequence(self.skips) ; r1.trim_quality(self.skips) ; r1.amend_header(r2.tag)
          r2.trim_sequence(self.skips) ; r2.trim_quality(self.skips) ; r2.amend_header(r1.tag)
          self.out1.write('%s\n%s\n+\n%s\n' % (r1.header, r1.sequence, r1.quality))
          self.out2.write('%s\n%s\n+\n%s\n' % (r2.header, r2.sequence, r2.quality))
